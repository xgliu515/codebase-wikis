...
function applyTheme(theme) {
  ...
  localStorage.setItem('hermes-agent-wiki-theme', theme);
  // highlight.js 主题切换
...
async function route() {
  ...
  if (!chapterId) {
    ...
    document.title = 'hermes-agent 中文参考 Wiki';
    return;
  }
  ...
  if (result) {
    ...
    document.title = `${result.chapter.num} · ${result.chapter.title} — hermes-agent Wiki`;
  } else {
    renderPageToc([], contentEl);
  }
}
...
async function main() {
  // 主题初始化
  const savedTheme = localStorage.getItem('hermes-agent-wiki-theme')
  ...
  document.getElementById('repo-root-btn').addEventListener('click', () => {
    ...
    const mode = cur ? '本地 VSCode' : `GitHub (hermes-agent@${VLLM_ANALYZED_COMMIT})`;
    const updated = prompt(
      `当前模式：${mode}\n\n` +
      `留空（默认）→ 跳到 GitHub 上对应 commit、对应行号\n` +
      `输入本地 hermes-agent 仓库绝对路径 → 跳到本地 VSCode（需先装好 VSCode）\n\n` +
      `路径示例：/Users/你的名字/git/hermes-agent`,
      cur
    );
  ...
  buildIndex().then(() => {
    initSearchUI();
    console.log('[hermes-agent Wiki] 搜索索引已就绪');
