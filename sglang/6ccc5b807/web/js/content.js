...
import { CHAPTER_BY_ID, TOURS, getRepoMode, ANALYZED_COMMIT, ANALYZED_TAG, ANALYZED_DATE, PROJECT_GITHUB_REPO } from './chapters.js';
...
export function renderHome(contentEl, chapters) {
  ...
  let html = `
    <div class="home-hero">
      <h1>SGLang 中文参考 Wiki</h1>
      <p class="lede">为深入学习 SGLang 源码、最终自己实现一个 LLM 推理引擎而写的可查询参考文档。</p>
      <div class="home-stats">
        <div class="stat"><strong>19</strong> 步导览 + <strong>15</strong> 章参考</div>
        ...
        <div class="stat">聚焦：<strong>推理引擎核心路径</strong></div>
      </div>
    </div>
    ...
    <section style="background:var(--accent-soft);border:1px solid var(--accent);border-radius:12px;padding:18px 22px;margin:24px 0 28px">
      <h2 style="margin:0 0 6px;font-size:20px;color:var(--accent);">推荐第一遍这样学：跟一次最简请求穿过 SGLang 全栈</h2>
      <p style="margin:0 0 12px;color:var(--text-soft);font-size:14px;">
        19 步导览，按 <strong>问题 → 朴素思路为何崩 → SGLang 怎么解决</strong> 的逻辑链展开。
        围绕 <code>llm.generate(["你好"], max_tokens=3)</code> 一个具体请求，逐层走完整个 SGLang。
      </p>
      ...
      <a href="#/tour-01-engine-init" style="display:inline-block;margin-left:8px;color:var(--accent);padding:8px 16px;border-radius:6px;text-decoration:none;font-size:14px;">或直接看第 1 步样品</a>
    </section>
    ...
    <section>
      <h2 style="font-size:20px;margin-bottom:8px;">单请求 Trace 导览（19 步）</h2>
      <p style="color:var(--text-soft);margin-top:0;font-size:14px;">
        每步约 150 行，按 8 段模板：当前情境 → 问题 → 朴素思路 → 为何崩 → SGLang 做法 → 代码位置 → 分支链接 → 学到了什么。
      </p>
