...
<svg viewBox="0 0 760 480" xmlns="http://www.w3.org/2000/svg" class="figure-svg" role="img" aria-label="OpenClaw 四层运行时架构：接入层、Gateway控制平面、消息编排、AI核心">
  <defs>
    <marker id="ar1" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
      <path d="M0,0 L10,5 L0,10 z" fill="#94a3b8"/>
    </marker>
  </defs>
  <text x="380" y="22" text-anchor="middle" font-size="14" font-weight="700" fill="currentColor">OpenClaw 四层运行时架构</text>
  <rect x="20" y="36" width="720" height="52" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1" rx="6"/>
  <text x="380" y="57" text-anchor="middle" font-size="11" font-weight="600" fill="#64748b">外部世界 —— 20+ 渠道</text>
  ...
  <text x="560" y="76" font-size="10" fill="#94a3b8">… 更多</text>
  ...
  <text x="380" y="122" text-anchor="middle" font-size="13" font-weight="700" fill="#ea580c">① 接入层 (channels)</text>
  <text x="380" y="140" text-anchor="middle" font-size="11" fill="#64748b">extensions/&lt;channel&gt;/  +  src/channels/plugins/</text>
  <text x="380" y="156" text-anchor="middle" font-size="11" fill="#64748b">每个渠道是一个插件，把平台原生消息归一化为 OpenClaw inbound 消息结构</text>
  ...
  <text x="380" y="218" text-anchor="middle" font-size="13" font-weight="700" fill="#0d9488">② Gateway 控制平面 (control plane)</text>
  <text x="380" y="236" text-anchor="middle" font-size="11" fill="#64748b">src/gateway/server.impl.ts</text>
  <text x="180" y="254" text-anchor="middle" font-size="10" fill="#64748b">HTTP / WebSocket 监听器</text>
  <text x="380" y="254" text-anchor="middle" font-size="10" fill="#64748b">RPC 方法注册表</text>
  <text x="580" y="254" text-anchor="middle" font-size="10" fill="#64748b">WS 连接生命周期 / auth / scope 鉴权</text>
  <text x="380" y="272" text-anchor="middle" font-size="10" fill="#64748b">内存注册表：chat run / session / 节点</text>
  ...
  <text x="380" y="322" text-anchor="middle" font-size="13" font-weight="700" fill="currentColor">③ 消息编排 (orchestration)</text>
  <text x="220" y="342" text-anchor="middle" font-size="10" fill="#64748b">src/auto-reply/  入站管线：去重、路由、节流</text>
  <text x="580" y="342" text-anchor="middle" font-size="10" fill="#64748b">src/sessions/  会话状态与历史</text>
  <text x="380" y="360" text-anchor="middle" font-size="10" fill="#64748b">src/auto-reply/reply/  出站投递（dispatcher）</text>
  ...
  <text x="380" y="418" text-anchor="middle" font-size="13" font-weight="700" fill="#7c3aed">④ AI 核心 (assistant)</text>
  <text x="180" y="438" text-anchor="middle" font-size="10" fill="#64748b">src/agents/  agent harness</text>
  <text x="380" y="438" text-anchor="middle" font-size="10" fill="#64748b">provider 插件  LLM</text>
  ...
  <text x="380" y="476" text-anchor="middle" font-size="9" fill="#94a3b8">LLM 推理 + 工具副作用 → 回复沿 ③→②→① 反向投递</text>
</svg>
<span class="figure-caption">图 R1.1 ｜ OpenClaw 四层运行时架构：消息自上而下经接入层、控制平面、编排层到 AI 核心，回复反向投递</span>
...
<svg viewBox="0 0 640 380" xmlns="http://www.w3.org/2000/svg" class="figure-svg" role="img" aria-label="进程启动引导链：五个阶段从 openclaw.mjs 到 server.impl.ts">
  <defs>
    <marker id="ar2" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
      <path d="M0,0 L10,5 L0,10 z" fill="#94a3b8"/>
    </marker>
  </defs>
  <text x="320" y="20" text-anchor="middle" font-size="14" font-weight="700" fill="currentColor">进程启动引导链（五阶段）</text>
  <rect x="60" y="32" width="520" height="54" fill="#fed7aa" stroke="#ea580c" stroke-width="1.5" rx="6"/>
  <text x="200" y="52" text-anchor="middle" font-size="12" font-weight="700" fill="#ea580c">① openclaw.mjs</text>
  <text x="200" y="68" text-anchor="middle" font-size="10" fill="#64748b">启动器（纯 .mjs，无构建依赖）</text>
  <text x="470" y="52" text-anchor="middle" font-size="10" fill="#64748b">Node 版本检查</text>
  <text x="470" y="66" text-anchor="middle" font-size="10" fill="#64748b">compile cache 决策 / respawn</text>
  ...
  <text x="200" y="120" text-anchor="middle" font-size="12" font-weight="700" fill="currentColor">② src/entry.ts</text>
  <text x="200" y="136" text-anchor="middle" font-size="10" fill="#64748b">入口（编译后为 dist/entry.js）</text>
  <text x="470" y="120" text-anchor="middle" font-size="10" fill="#64748b">isMainModule 守卫</text>
  <text x="470" y="136" text-anchor="middle" font-size="10" fill="#64748b">参数规范化 / profile / container / trace</text>
  ...
  <text x="200" y="188" text-anchor="middle" font-size="12" font-weight="700" fill="currentColor">③ src/cli/run-main.ts</text>
  <text x="200" y="204" text-anchor="middle" font-size="10" fill="#64748b">命令路由</text>
  <text x="470" y="188" text-anchor="middle" font-size="10" fill="#64748b">容器 / profile / dotenv</text>
  ...
  <text x="200" y="256" text-anchor="middle" font-size="12" font-weight="700" fill="#0d9488">④ gateway 命令</text>
  <text x="200" y="272" text-anchor="middle" font-size="10" fill="#64748b">src/cli/gateway-cli/run.ts</text>
  <text x="470" y="264" text-anchor="middle" font-size="10" fill="#64748b">解析 --port / --bind / --auth 选项</text>
  ...
  <text x="200" y="324" text-anchor="middle" font-size="12" font-weight="700" fill="#7c3aed">⑤ server.ts → server.impl.ts</text>
  <text x="200" y="340" text-anchor="middle" font-size="10" fill="#64748b">真正启动 Gateway</text>
  <text x="470" y="332" text-anchor="middle" font-size="10" fill="#64748b">延迟加载 server.impl.ts</text>
  <text x="470" y="348" text-anchor="middle" font-size="10" fill="#64748b">HTTP/WS 监听端口 18789</text>
</svg>
<span class="figure-caption">图 R1.2 ｜ 进程启动引导链五阶段：从纯 .mjs 启动器到 Gateway 端口监听</span>
...
<svg viewBox="0 0 760 420" xmlns="http://www.w3.org/2000/svg" class="figure-svg" role="img" aria-label="openclaw gateway run 完整调用链树形图">
  <defs>
    <marker id="ar3" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
      <path d="M0,0 L10,5 L0,10 z" fill="#94a3b8"/>
    </marker>
  </defs>
  <text x="380" y="20" text-anchor="middle" font-size="14" font-weight="700" fill="currentColor">openclaw gateway run —— 完整调用链</text>
  <rect x="20" y="34" width="160" height="36" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1" rx="4"/>
  <text x="100" y="50" text-anchor="middle" font-size="11" font-weight="600" fill="currentColor">shell</text>
  <text x="100" y="64" text-anchor="middle" font-size="10" fill="#94a3b8">node openclaw.mjs</text>
  <line x1="100" y1="70" x2="100" y2="86" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar3)"/>
  <rect x="20" y="86" width="280" height="52" fill="#fed7aa" stroke="#ea580c" stroke-width="1.5" rx="5"/>
  <text x="160" y="104" text-anchor="middle" font-size="12" font-weight="700" fill="#ea580c">openclaw.mjs  [纯 .mjs 启动器]</text>
  <text x="160" y="120" text-anchor="middle" font-size="10" fill="#64748b">Node 版本检查 + compile cache + (可能 respawn)</text>
  <text x="160" y="132" text-anchor="middle" font-size="10" fill="#64748b">→ import dist/entry.js</text>
  <line x1="160" y1="138" x2="160" y2="154" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar3)"/>
  <rect x="20" y="154" width="320" height="52" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1.5" rx="5"/>
  <text x="180" y="172" text-anchor="middle" font-size="12" font-weight="700" fill="currentColor">src/entry.ts  [dist/entry.js]</text>
  <text x="180" y="188" text-anchor="middle" font-size="10" fill="#64748b">isMainModule 守卫 + argv 规范化 + profile/container</text>
  <text x="180" y="200" text-anchor="middle" font-size="10" fill="#64748b">→ runMainOrRootHelp → import dist/cli/run-main.js</text>
  <line x1="200" y1="206" x2="200" y2="222" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar3)"/>
  <rect x="20" y="222" width="360" height="52" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1.5" rx="5"/>
  <text x="200" y="240" text-anchor="middle" font-size="12" font-weight="700" fill="currentColor">src/cli/run-main.ts</text>
  <text x="200" y="256" text-anchor="middle" font-size="10" fill="#64748b">runCli: 容器 / profile / dotenv / 代理</text>
  <text x="200" y="268" text-anchor="middle" font-size="10" fill="#64748b">→ gateway run fast-path（或完整 Commander 路径）</text>
  <line x1="220" y1="274" x2="220" y2="290" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar3)"/>
  <rect x="20" y="290" width="400" height="42" fill="#99f6e4" stroke="#0d9488" stroke-width="1.5" rx="5"/>
  <text x="220" y="308" text-anchor="middle" font-size="12" font-weight="700" fill="#0d9488">src/cli/gateway-cli/run.ts</text>
  <text x="220" y="324" text-anchor="middle" font-size="10" fill="#64748b">解析 --port / --bind / --auth → startGatewayServer</text>
  <line x1="240" y1="332" x2="240" y2="348" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar3)"/>
  <rect x="20" y="348" width="400" height="32" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1" rx="5"/>
  <text x="220" y="364" text-anchor="middle" font-size="11" font-weight="600" fill="currentColor">src/gateway/server.ts</text>
  <text x="220" y="376" text-anchor="middle" font-size="10" fill="#64748b">延迟加载 → import server.impl.js</text>
  <line x1="260" y1="380" x2="260" y2="396" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar3)"/>
  <rect x="20" y="396" width="520" height="18" fill="#ddd6fe" stroke="#7c3aed" stroke-width="1.5" rx="4"/>
  <text x="280" y="409" text-anchor="middle" font-size="11" font-weight="700" fill="#7c3aed">src/gateway/server.impl.ts  → HTTP/WS 监听端口 18789</text>
</svg>
<span class="figure-caption">图 R1.3 ｜ openclaw gateway run 完整调用链：从 shell 到端口监听的五级树形路径</span>
