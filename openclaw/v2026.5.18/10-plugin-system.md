...
<svg viewBox="0 0 760 280" xmlns="http://www.w3.org/2000/svg" class="figure-svg" role="img" aria-label="插件 SDK 两级结构：发布包 packages/plugin-sdk 转发到 core 实现 src/plugin-sdk">
  <defs>
    <marker id="ar1" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
      <path d="M0,0 L10,5 L0,10 z" fill="#94a3b8"/>
    </marker>
  </defs>
  <rect x="40" y="20" width="680" height="100" rx="8" fill="#fed7aa" stroke="#ea580c" stroke-width="1.5"/>
  <text x="380" y="44" text-anchor="middle" font-size="13" font-weight="700" fill="#ea580c">packages/plugin-sdk/</text>
  <text x="380" y="62" text-anchor="middle" font-size="11" fill="#94a3b8">发布出去的 npm 包  @openclaw/plugin-sdk</text>
  <text x="380" y="80" text-anchor="middle" font-size="11" fill="#64748b">src/*.ts  ← 多数文件只是一行 re-export，转发到下面</text>
  <text x="380" y="98" text-anchor="middle" font-size="11" fill="#64748b">package.json exports  ← 一长串子路径映射</text>
  <line x1="380" y1="120" x2="380" y2="158" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar1)"/>
  <rect x="40" y="158" width="680" height="106" rx="8" fill="#99f6e4" stroke="#0d9488" stroke-width="1.5"/>
  <text x="380" y="182" text-anchor="middle" font-size="13" font-weight="700" fill="#0d9488">src/plugin-sdk/</text>
  <text x="380" y="200" text-anchor="middle" font-size="11" fill="#94a3b8">SDK 的真正实现（在 core 仓库里）</text>
  <text x="380" y="218" text-anchor="middle" font-size="11" fill="#64748b">index.ts  ← 极简根 surface</text>
  <text x="380" y="234" text-anchor="middle" font-size="11" fill="#64748b">plugin-entry.ts  ← definePluginEntry 等入口辅助</text>
  <text x="380" y="250" text-anchor="middle" font-size="11" fill="#64748b">core.ts / *-entry.ts  ← channel/provider 入口契约</text>
  <text x="380" y="264" text-anchor="middle" font-size="10" fill="#94a3b8">... ~数百个文件</text>
</svg>
<span class="figure-caption">图 R10.1 ｜ 插件 SDK 两级结构：packages/plugin-sdk 是发布外壳，真正实现在 src/plugin-sdk</span>
...
<svg viewBox="0 0 760 500" xmlns="http://www.w3.org/2000/svg" class="figure-svg" role="img" aria-label="插件加载完整流程：扫盘候选到 manifest 读取到启用决策到注册到钩子运行器">
  <defs>
    <marker id="ar1" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
      <path d="M0,0 L10,5 L0,10 z" fill="#94a3b8"/>
    </marker>
  </defs>
  <rect x="140" y="10" width="480" height="50" rx="6" fill="#fed7aa" stroke="#ea580c" stroke-width="1.5"/>
  <text x="380" y="32" text-anchor="middle" font-size="13" font-weight="700" fill="#ea580c">discoverOpenClawPlugins()</text>
  <text x="380" y="50" text-anchor="middle" font-size="10" fill="#94a3b8">扫盘，得到候选</text>
  <line x1="380" y1="60" x2="380" y2="90" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar1)"/>
  <rect x="140" y="90" width="480" height="50" rx="6" fill="#ddd6fe" stroke="#7c3aed" stroke-width="1.5"/>
  <text x="380" y="112" text-anchor="middle" font-size="13" font-weight="700" fill="#7c3aed">loadPluginManifestRegistry()</text>
  <text x="380" y="130" text-anchor="middle" font-size="10" fill="#94a3b8">读所有 manifest，得到 PluginManifestRegistry</text>
  <line x1="380" y1="140" x2="380" y2="170" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar1)"/>
  <rect x="140" y="170" width="480" height="62" rx="6" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1"/>
  <text x="380" y="194" text-anchor="middle" font-size="12" font-weight="600" fill="currentColor">resolveEffectiveEnableState()</text>
  <text x="380" y="212" text-anchor="middle" font-size="11" fill="#64748b">resolveEffectivePluginActivationState()</text>
  <text x="380" y="228" text-anchor="middle" font-size="10" fill="#94a3b8">按配置/默认策略决定每个插件启用/禁用</text>
  <line x1="380" y1="232" x2="380" y2="262" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar1)"/>
  <rect x="80" y="262" width="600" height="100" rx="6" fill="#99f6e4" stroke="#0d9488" stroke-width="1.5"/>
  <text x="380" y="284" text-anchor="middle" font-size="12" font-weight="700" fill="#0d9488">对每个启用的插件：</text>
  <text x="380" y="304" text-anchor="middle" font-size="11" fill="#64748b">buildPluginApi(...)  ← 按 registrationMode 构造 api（注入真/假 handlers）</text>
  <text x="380" y="320" text-anchor="middle" font-size="11" fill="#64748b">动态 import 插件入口模块</text>
  <text x="380" y="338" text-anchor="middle" font-size="11" fill="#64748b">调 entry.register(api)  ← 插件执行注册</text>
  <text x="380" y="354" text-anchor="middle" font-size="10" fill="#94a3b8">src/plugins/loader.ts</text>
  <line x1="380" y1="362" x2="380" y2="392" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar1)"/>
  <rect x="140" y="392" width="480" height="44" rx="6" fill="#f1f5f9" stroke="#94a3b8" stroke-width="1"/>
  <text x="380" y="414" text-anchor="middle" font-size="12" font-weight="600" fill="currentColor">注册结果汇入 PluginRegistry</text>
  <line x1="380" y1="436" x2="380" y2="460" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar1)"/>
  <rect x="140" y="460" width="480" height="34" rx="6" fill="#fef3c7" stroke="#f59e0b" stroke-width="1"/>
  <text x="380" y="482" text-anchor="middle" font-size="12" font-weight="600" fill="#92400e">initializeGlobalHookRunner(...)  ← 钩子运行器就绪</text>
</svg>
<span class="figure-caption">图 R10.2 ｜ 插件加载完整流程：扫盘 → 读 manifest → 启用决策 → 逐插件注册 → 钩子运行器初始化</span>
...
<svg viewBox="0 0 760 220" xmlns="http://www.w3.org/2000/svg" class="figure-svg" role="img" aria-label="内置 bundled 插件与外部 official 插件对比">
  <rect x="20" y="20" width="340" height="180" rx="8" fill="#fed7aa" stroke="#ea580c" stroke-width="1.5"/>
  <text x="190" y="46" text-anchor="middle" font-size="13" font-weight="700" fill="#ea580c">内置 bundled 插件</text>
  <text x="190" y="72" text-anchor="middle" font-size="11" fill="#64748b">随 core dist 一起打包发布</text>
  <text x="190" y="92" text-anchor="middle" font-size="11" fill="#64748b">例如 core 必备的能力插件</text>
  <text x="190" y="130" text-anchor="middle" font-size="10" fill="#94a3b8">可用 "bundled-only facade loader"</text>
  <text x="190" y="150" text-anchor="middle" font-size="10" fill="#94a3b8">AGENTS.md:34</text>
  <rect x="400" y="20" width="340" height="180" rx="8" fill="#99f6e4" stroke="#0d9488" stroke-width="1.5"/>
  <text x="570" y="46" text-anchor="middle" font-size="13" font-weight="700" fill="#0d9488">外部 official 插件</text>
  <text x="570" y="72" text-anchor="middle" font-size="11" fill="#64748b">独立 npm 包，不进 core dist</text>
  <text x="570" y="92" text-anchor="middle" font-size="11" fill="#64748b">例如可选 channel / provider</text>
  <text x="570" y="130" text-anchor="middle" font-size="10" fill="#94a3b8">core 用 registry-aware</text>
  <text x="570" y="148" text-anchor="middle" font-size="10" fill="#94a3b8">facade-runtime 或通用契约</text>
  <text x="570" y="166" text-anchor="middle" font-size="10" fill="#94a3b8">AGENTS.md:35</text>
  <line x1="360" y1="110" x2="400" y2="110" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="4,3"/>
  <text x="380" y="107" text-anchor="middle" font-size="10" fill="#94a3b8">vs</text>
</svg>
<span class="figure-caption">图 R10.3 ｜ 插件两级发布：bundled 内置随 core 发布，external official 独立 npm 包按需安装</span>
...
<svg viewBox="0 0 760 620" xmlns="http://www.w3.org/2000/svg" class="figure-svg" role="img" aria-label="插件系统全章总览：磁盘插件包到加载器到注册中心到运行时钩子">
  <defs>
    <marker id="ar1" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
      <path d="M0,0 L10,5 L0,10 z" fill="#94a3b8"/>
    </marker>
  </defs>
  <rect x="20" y="10" width="720" height="120" rx="8" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1"/>
  <text x="380" y="32" text-anchor="middle" font-size="13" font-weight="700" fill="currentColor">磁盘 extensions/*/</text>
  <text x="380" y="50" text-anchor="middle" font-size="10" fill="#94a3b8">(134 个插件包)</text>
  <rect x="40" y="58" width="340" height="58" rx="5" fill="#ddd6fe" stroke="#7c3aed" stroke-width="1"/>
  <text x="210" y="76" text-anchor="middle" font-size="11" font-weight="600" fill="#7c3aed">manifest 元数据（不执行代码即可读）</text>
  <text x="210" y="92" text-anchor="middle" font-size="10" fill="#64748b">openclaw.plugin.json</text>
  <text x="210" y="108" text-anchor="middle" font-size="10" fill="#64748b">package.json (openclaw 块)  ·  index.ts (definePluginEntry)</text>
  <rect x="400" y="58" width="320" height="58" rx="5" fill="#fed7aa" stroke="#ea580c" stroke-width="1"/>
  <text x="560" y="76" text-anchor="middle" font-size="11" font-weight="600" fill="#ea580c">公共 barrel（唯一对外 ABI）</text>
  <text x="560" y="92" text-anchor="middle" font-size="10" fill="#64748b">api.ts  ·  runtime-api.ts</text>
  <text x="560" y="108" text-anchor="middle" font-size="10" fill="#64748b">src/  (私有实现，外界不准 deep-import)</text>
  <line x1="380" y1="130" x2="380" y2="158" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar1)"/>
  <text x="440" y="150" font-size="10" fill="#64748b">discoverOpenClawPlugins()  ← 扫盘，得候选</text>
  <rect x="60" y="158" width="640" height="120" rx="8" fill="#99f6e4" stroke="#0d9488" stroke-width="1.5"/>
  <text x="380" y="180" text-anchor="middle" font-size="13" font-weight="700" fill="#0d9488">插件加载器  src/plugins/</text>
  <text x="380" y="198" text-anchor="middle" font-size="11" fill="#64748b">loadPluginManifestRegistry()  —— 读 manifest</text>
  <text x="380" y="216" text-anchor="middle" font-size="11" fill="#64748b">resolveEffectiveEnableState()  —— 决定启用/禁用</text>
  <text x="380" y="234" text-anchor="middle" font-size="11" fill="#64748b">buildPluginApi(mode)  —— 按模式造 api</text>
  <text x="380" y="252" text-anchor="middle" font-size="11" fill="#64748b">import 入口 + entry.register(api)</text>
  <line x1="380" y1="278" x2="380" y2="308" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar1)"/>
  <text x="440" y="298" font-size="10" fill="#64748b">api.registerTool / Channel / Provider / Hook...</text>
  <rect x="60" y="308" width="640" height="100" rx="8" fill="#ddd6fe" stroke="#7c3aed" stroke-width="1.5"/>
  <text x="380" y="332" text-anchor="middle" font-size="13" font-weight="700" fill="#7c3aed">PluginRegistry  src/plugins/registry.ts</text>
  <text x="380" y="350" text-anchor="middle" font-size="11" fill="#64748b">工具 → agent 工具表    channel → 渠道运行时</text>
  <text x="380" y="368" text-anchor="middle" font-size="11" fill="#64748b">provider → 推理循环    hook → HookRunner</text>
  <text x="380" y="386" text-anchor="middle" font-size="11" fill="#64748b">gatewayMethod / command / httpRoute → ...</text>
  <text x="380" y="400" text-anchor="middle" font-size="10" fill="#94a3b8">src/plugins/registry.ts</text>
  <line x1="380" y1="408" x2="380" y2="436" stroke="#94a3b8" stroke-width="1.2" marker-end="url(#ar1)"/>
  <text x="445" y="428" font-size="10" fill="#64748b">initializeGlobalHookRunner()</text>
  <rect x="60" y="436" width="640" height="60" rx="8" fill="#fef3c7" stroke="#f59e0b" stroke-width="1.5"/>
  <text x="380" y="458" text-anchor="middle" font-size="12" font-weight="700" fill="#92400e">运行时 HookRunner</text>
  <text x="380" y="476" text-anchor="middle" font-size="11" fill="#64748b">message_received / before_tool_call / agent_end / gateway_start ... 等 36 个生命周期点触发插件钩子</text>
  <line x1="380" y1="496" x2="380" y2="514" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2"/>
  <rect x="20" y="514" width="340" height="80" rx="6" fill="#f1f5f9" stroke="#cbd5e1" stroke-width="1"/>
  <text x="190" y="534" text-anchor="middle" font-size="11" font-weight="600" fill="currentColor">catalog</text>
  <text x="190" y="552" text-anchor="middle" font-size="10" fill="#64748b">scripts/lib/official-external-*-catalog.json</text>
  <text x="190" y="570" text-anchor="middle" font-size="10" fill="#64748b">core 在不装插件前提下知道</text>
  <text x="190" y="586" text-anchor="middle" font-size="10" fill="#64748b">"有哪些可选插件"</text>
  <rect x="400" y="514" width="340" height="80" rx="6" fill="#fef2f2" stroke="#f87171" stroke-width="1"/>
  <text x="570" y="534" text-anchor="middle" font-size="11" font-weight="600" fill="#dc2626">架构边界</text>
  <text x="570" y="552" text-anchor="middle" font-size="10" fill="#64748b">插件 prod 代码 → 只能 openclaw/plugin-sdk/* + 自己 barrel</text>
  <text x="570" y="570" text-anchor="middle" font-size="10" fill="#64748b">core/测试 → 只能插件 api.ts（不准 deep-import src/**）</text>
  <text x="570" y="586" text-anchor="middle" font-size="10" fill="#64748b">src/plugins/contracts/ 契约测试强制执行</text>
</svg>
<span class="figure-caption">图 R10.4 ｜ 插件系统全章总览：磁盘插件包 → 加载器 → PluginRegistry → HookRunner，catalog 与架构边界并列</span>
